...
bool ShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	if (!InitializeShader(device, hwnd, L"Shader.vs", L"Shader.ps"))
		return false;
...
//描画関数
bool ShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, D3DXMATRIX worldMatrix,
				...
				D3DXVECTOR3 cameraPosition,D3DXVECTOR4 specularColor,float specularPower)
{
	...
	if (!SetShaderVariables(deviceContext, worldMatrix, viewMatrix, projectionMatrix,texture,LightDirection,diffuseColor,ambientColor,
...
bool ShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	D3D11_INPUT_ELEMENT_DESC polygon_layout[3];	//ポリゴンレイアウト
	...
	D3D11_BUFFER_DESC light_buffer_desc;		//ピクセルシェーダのライト定数バッファ
	D3D11_BUFFER_DESC camera_buffer_desc;		//頂点シェーダのカメラ定数バッファ
	...
	//ピクセルシェーダにあるライト定数バッファの記述のセットアップ
	//D3D11_BIND_CONSTANT_BUFFERを使用する場合、ByteWidthは常に16の倍数でなければならない。でないと、失敗する
	light_buffer_desc.Usage = D3D11_USAGE_DYNAMIC;			//バッファーで想定されている読み込みおよび書き込みの方法
	light_buffer_desc.ByteWidth = sizeof(LightBufferType);		//バッファのサイズ
	light_buffer_desc.BindFlags = D3D11_BIND_CONSTANT_BUFFER;	//バッファーをどのようにパイプラインにバインドするか
	light_buffer_desc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;		//CPU アクセスのフラグ
	light_buffer_desc.MiscFlags = 0;							//その他のフラグ
	light_buffer_desc.StructureByteStride = 0;							//構造体が構造化バッファーを表す場合、その構造体のサイズ
	...
	//頂点シェーダにあるカメラ定数バッファの記述のセットアップ
	//D3D11_BIND_CONSTANT_BUFFERを使用する場合、ByteWidthは常に16の倍数でなければならない。でないと、失敗する
	camera_buffer_desc.Usage = D3D11_USAGE_DYNAMIC;			//バッファーで想定されている読み込みおよび書き込みの方法
	camera_buffer_desc.ByteWidth = sizeof(LightBufferType);		//バッファのサイズ
	camera_buffer_desc.BindFlags = D3D11_BIND_CONSTANT_BUFFER;	//バッファーをどのようにパイプラインにバインドするか
	camera_buffer_desc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;		//CPU アクセスのフラグ
	camera_buffer_desc.MiscFlags = 0;							//その他のフラグ
	camera_buffer_desc.StructureByteStride = 0;							//構造体が構造化バッファーを表す場合、その構造体のサイズ

	//このクラス内から頂点シェーダ定数バッファにアクセスできるように、定数バッファポインタを作成
	result = device->CreateBuffer(&camera_buffer_desc, NULL, &m_camera_buffer);
	if (FAILED(result))
		return false;
...
void ShaderClass::OutputShaderErrorMessage(ID3D10Blob* errorMassage, HWND hwnd, WCHAR* shaderFilename)
{
...
bool ShaderClass::SetShaderVariables(ID3D11DeviceContext* deviceContext, D3DXMATRIX worldMatrix,
								...
								D3DXVECTOR3 cameraPosition, D3DXVECTOR4 specularColor, float specularPower)
{
	...
	LightBufferType*  data_ptr2;				//ライト定数バッファ内のデータへのポインタ
	CameraBufferType* data_ptr3;				//カメラ定数バッファ内のデータへのポインタ
	...
	//カメラ定数バッファをロックして書き込み可能にする
	result = deviceContext->Map(m_camera_buffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mapped_resource);
	if (FAILED(result))
		return false;

	//定数バッファ内のデータへのポインタを取得
	data_ptr3 = (CameraBufferType*)mapped_resource.pData;

	//カメラの位置を定数バッファにコピー
	data_ptr3->cameraPosition = cameraPosition;
	data_ptr3->padding = 0.0f;

	//カメラ定数バッファのロックを解除
	deviceContext->Unmap(m_camera_buffer, 0);

	//頂点シェーダのカメラ定数バッファの位置を設定(頂点シェーダの２番目のバッファ)
	buffer_number = 1;

	//更新された値で頂点シェーダのカメラ定数バッファを設定
	deviceContext->VSSetConstantBuffers(buffer_number, 1, &m_camera_buffer);

	//ピクセルシェーダにシェーダテクスチャリソースを設定
	deviceContext->PSSetShaderResources(0, 1, &texture);

	//書き込み可能にするためにライト定数バッファをロックする
	result = deviceContext->Map(m_light_buffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mapped_resource);
	if (FAILED(result))
		return false;

	//ライト定数バッファ内のデータへのポインタを取得
	data_ptr2 = (LightBufferType*)mapped_resource.pData;

	//照明変数をライト定数バッファにコピー
	data_ptr2->ambientColor = ambientColor;
	data_ptr2->diffuseColor = diffuseColor;
	data_ptr2->lightDirection = LightDirection;
	data_ptr2->specularColor = specularColor;
	data_ptr2->specularPower = specularPower;

	//ライト定数バッファのロックを解除
	deviceContext->Unmap(m_light_buffer, 0);

	//ピクセルシェーダのライト定数バッファの位置を設定
	buffer_number = 0;

	//最後に、更新された値でピクセルシェーダのライト定数バッファを設定
	deviceContext->PSSetConstantBuffers(buffer_number, 1, &m_light_buffer);
