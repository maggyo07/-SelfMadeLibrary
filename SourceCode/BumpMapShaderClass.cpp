...
//初期化関数
bool BumpMapShaderClass::Initialize(const HWND& hwnd)
{
	//頂点シェーダとピクセルシェーダを初期化する
	if (!InitializeShader(hwnd, L"Bumpmap.vs", L"Bumpmap.ps"))
		return false;
...
//描画関数
bool BumpMapShaderClass::Render(const int& indexCount, const D3DXMATRIX& worldMatrix,
	const D3DXMATRIX& viewMatrix, const D3DXMATRIX& projectionMatrix, ID3D11ShaderResourceView** texture,D3DXVECTOR3 light_direction,D3DXVECTOR4 diffuse_color)
{
...
void BumpMapShaderClass::OutputShaderErrorMessage(ID3D10Blob* errorMassage, HWND hwnd, WCHAR* shaderFilename)
{
...
bool BumpMapShaderClass::SetShaderParameters(D3DXMATRIX worldMatrix,
	D3DXMATRIX viewMatrix, D3DXMATRIX projextionMatrix, ID3D11ShaderResourceView** texture, D3DXVECTOR3 light_direction, D3DXVECTOR4 diffuse_color)
{
	...
	//ピクセルシェーダ内の定数バッファの位置を設定
	buffer_number = 0;

	//更新された値でピクセルシェーダの行列定数バッファをFinanlyに設定
	...
	//今すぐ更新された値でピクセルシェーダーのライトバッファを設定
