				...
				D3DXVECTOR4 pixelColor)
{
	...
	if (!SetShaderVariables(deviceContext, worldMatrix, viewMatrix, projectionMatrix,texture,pixelColor))
		return false;
...
bool FontShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	D3D11_BUFFER_DESC pixel_buffer_desc;		//ピクセルシェーダのピクセルカラー定数バッファ
	...
	//ピクセルシェーダにあるピクセルカラー定数バッファの記述をセットアップ
	pixel_buffer_desc.Usage = D3D11_USAGE_DYNAMIC;			//バッファーで想定されている読み込みおよび書き込みの方法
	pixel_buffer_desc.ByteWidth = sizeof(PixelBufferType);		//バッファのサイズ
	pixel_buffer_desc.BindFlags = D3D11_BIND_CONSTANT_BUFFER;	//バッファーをどのようにパイプラインにバインドするか
	pixel_buffer_desc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;		//CPU アクセスのフラグ
	pixel_buffer_desc.MiscFlags = 0;							//その他のフラグ
	pixel_buffer_desc.StructureByteStride = 0;							//構造体が構造化バッファーを表す場合、その構造体のサイズ

	//このクラス内からピクセルシェーダ定数バッファにアクセスできるように、定数バッファポインタを作成
	result = device->CreateBuffer(&pixel_buffer_desc, NULL, &m_pixel_buffer);
	if (FAILED(result))
		return false;
...
void FontShaderClass::OutputShaderErrorMessage(ID3D10Blob* errorMassage, HWND hwnd, WCHAR* shaderFilename)
{
...
bool FontShaderClass::SetShaderVariables(ID3D11DeviceContext* deviceContext, D3DXMATRIX worldMatrix,
								...
								D3DXVECTOR4 pixelColor)
{
	...
	PixelBufferType*  data_ptr2;				//ピクセルカラー定数バッファ内のデータへのポインタ
	...
	//書き込むことができるようにピクセルカラー定数バッファをロック
	result = deviceContext->Map(m_pixel_buffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mapped_resource);
	if (FAILED(result))
		return false;

	//ピクセルカラー定数バッファ内のデータへのポインタを取得
	data_ptr2 = (PixelBufferType*)mapped_resource.pData;

	//ピクセルカラーをピクセルカラー定数バッファにコピー
	data_ptr2->pixelColor = pixelColor;

	//ピクセルカラー定数バッファのロックを解除
	deviceContext->Unmap(m_pixel_buffer, 0);

	//ピクセルシェーダのピクセルカラー定数バッファの位置を設定
	buffer_number = 0;

	//更新された値でピクセルシェーダのピクセルカラー定数バッファを設定
	deviceContext->PSSetConstantBuffers(buffer_number, 1, &m_pixel_buffer);
